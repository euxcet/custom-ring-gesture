...
class ExpBaselineWithSamplesDataset(Dataset):
    def __init__(
        self,
        dataset_type: str,
        x_files: list[str],
        ...
        self.num_classes = len(samples) + 1
        self.do_aug = do_aug
        ...
        self.xs, self.ys = self.filter_data(self.xs, self.ys, [0], keep_index=False)

        self.xs, self.ys = self.add_samples(self.xs, self.ys, samples)
    ...
    def add_samples(self, xs: np.ndarray, ys: np.ndarray, samples: list[list[torch.Tensor]]) -> tuple[np.ndarray, np.ndarray]:
        new_xs = []
        new_ys = []
        for i in range(len(samples)):
            for sample in samples[i]:
                new_xs.append(sample.astype(np.float32))
                new_ys.append(i + 1)
        return np.concatenate([xs, new_xs]), np.concatenate([ys, new_ys])
